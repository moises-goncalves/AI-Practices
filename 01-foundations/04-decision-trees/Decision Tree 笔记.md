...
决策树通过一系列"如果特征满足某条件，则走左/右子树"的规则，将特征空间划分为若干区域。每一个叶子节点对应一个区域：
...
- 使用**贪心策略**：在当前节点上，枚举特征及其阈值，选择可最大化"纯度提升"的划分
...
| 准则 | 计算复杂度 | 效果 | 使用建议 |
|------|-----------|------|----------|
| Gini | 较低 | 通常与 Entropy 接近 | 默认选择 |
| Entropy | 较高 | 更贴近信息增益概念 | 特殊场景可尝试 |
...
| 阶段 | 复杂度 | 说明 |
|------|--------|------|
| 训练 | $O(m n \log m)$ | $m$ 为样本数，$n$ 为特征数 |
| 预测 | $O(\log m)$ | 沿树从根走到叶，非常快 |
...
| 参数 | 说明 | 推荐范围 |
|------|------|----------|
| `max_depth` | 树的最大深度 | 3-15 |
| `min_samples_split` | 内部节点最少样本数 | 2-20 |
| `min_samples_leaf` | 叶子节点最少样本数 | 1-10 |
| `max_leaf_nodes` | 最大叶子数量 | 根据数据规模 |
...
其中 $R(T)$ 为原树的经验风险，$\lvert T\rvert$ 为叶子数。增大 $\alpha$ 会鼓励更小更简单的树。

**典型流程**：

1. 使用 `DecisionTreeClassifier.cost_complexity_pruning_path()` 得到一系列候选 $\alpha$
...
| 特性 | 决策树回归 | 线性回归 |
|------|-----------|----------|
| 非线性建模 | 可自然表达强非线性关系 | 需要特征工程 |
| 平滑性 | 缺乏平滑性，对小扰动不连续 | 平滑连续 |
| 外推能力 | 无法外推 | 可以外推 |
...
- 决策树仅使用形式如 `x_j <= threshold` 的轴平行分裂
...
|------|------|
| 算法本质 | CART 贪心递归二叉分裂 |
| 分类准则 | Gini 不纯度（默认）或信息熵 |
| 回归准则 | MSE 最小化 |
| 主要问题 | 高方差、易过拟合、轴平行限制 |
| 解决方案 | 正则化（预剪枝/后剪枝）或集成方法 |
| 核心优势 | 可解释性、无需特征缩放 |

通过本章学习，应当深刻理解"决策树是高方差、易过拟合、但非常可解释的基学习器"这一核心结论，为后续学习随机森林和梯度提升奠定基础。
