...
for some bounded potential function Φ: S → ℝ.
...
class DynamicShapingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.adaptation_method not in ("exponential", "linear"):
            raise ValueError(f"Unknown adaptation_method: {self.adaptation_method}")
...
def verify_policy_invariance(
    ...
    stats = {
        ...
        "return_difference_mean": float(np.mean(shaped_returns - original_returns)),
    }
...
if __name__ == "__main__":
    print("=" * 70)
    print("Reward Shaping Module - Validation Tests")
    print("=" * 70)

    np.random.seed(42)

    # Test 1: Distance-based shaper
    ...
    for state in test_states:
        ...
        print(f"  State {state}: Φ(s) = {p:.4f}")

    # Test 2: Shaping bonus
    print("\n[Test 2] Shaping Bonus")
    ...
    # Test 3: Adaptive shaper
    print("\n[Test 3] Adaptive Reward Shaper")
    adaptive = AdaptiveRewardShaper(
        base_shaper=shaper,
        dynamic_config=DynamicShapingConfig(
            initial_weight=1.0, decay_rate=0.99, min_weight=0.01
        ),
    )

    for _ in range(100):
        adaptive.compute_shaping_bonus(
            np.array([0.0, 0.0]), np.array([1.0, 1.0])
        )
    print(f"  Weight after 100 steps: {adaptive.current_weight:.4f}")
