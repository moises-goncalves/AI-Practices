...
算法历史与背景:
    动态规划方法源于Richard Bellman在1950年代提出的最优化理论。这些算法
    是强化学习的理论基础，虽然在实践中受限于需要完整的环境模型和状态空间
    爆炸问题，但为后续的无模型方法（如Q-Learning、SARSA）提供了重要的
    理论支撑。
...
class DynamicProgrammingSolver:
    ...
    def evaluate_policy(
        ...
        算法背景:
            策略评估是动态规划的基础算子，用于计算给定策略的价值函数。该算法
            通过不断将当前价值估计代入贝尔曼期望方程的右侧，产生新的价值估计，
            直到收敛。这是一个典型的不动点迭代过程。

        为什么需要策略评估:
            在策略迭代算法中，我们需要评估当前策略的好坏，才能进行改进。
            策略评估提供了一种系统化的方法来计算策略的价值函数，为策略改进
            提供依据。
        ...
        算法核心:
            ...
            采用同步更新方式，每次迭代更新所有状态的价值，直到最大变化量
            小于阈值θ。
    ...
    def improve_policy(self, V: ValueFunction) -> Policy:
        ...
        算法背景:
            策略改进定理保证，对于任意策略π，如果我们贪婪地选择使Q值最大的
            动作构造新策略π'，则新策略至少不比原策略差，即V^π' ≥ V^π。
            这一定理是策略迭代算法收敛性的理论基础。

        为什么需要策略改进:
            策略评估告诉我们当前策略的价值，但不告诉我们如何改进。策略改进
            利用价值函数的信息，贪心地选择最优动作，系统化地提升策略性能。

        算法核心:
            对每个状态选择使动作价值最大的动作：
                π'(s) = argmax_a Σ_{s'} P(s'|s,a) [R + γV(s')]

            这等价于在每个状态选择最大化Q值的动作，构造确定性最优策略。

        策略改进定理:
            若 Q^π(s, π'(s)) ≥ V^π(s) 对所有s成立，则 V^π'(s) ≥ V^π(s)
            证明基于贝尔曼期望方程的递归展开。
    ...
    def policy_iteration(self, max_iterations: int = 100) -> AlgorithmResult:
        ...
        算法背景:
            策略迭代由Howard于1960年提出，是求解MDP的经典方法。该算法交替
            执行策略评估和策略改进两个步骤，利用策略改进定理保证单调收敛至
            最优策略。策略迭代通常在较少的外层迭代次数内收敛，但每次迭代需要
            完全求解策略评估问题。

        为什么提出策略迭代:
            动态规划需要同时求解最优策略和最优价值函数两个问题。策略迭代将
            这个复杂问题分解为两个相对简单的子问题：给定策略求价值（评估），
            给定价值求策略（改进）。通过交替求解这两个子问题，最终收敛到最优解。

        算法流程：
            1. 初始化随机策略 π_0
            2. 策略评估：求解 V^π_k（完全求解贝尔曼期望方程）
            3. 策略改进：π_{k+1} = greedy(V^π_k)
            4. 若 π_{k+1} = π_k 则停止，否则 k←k+1 转步骤2

        收敛性保证:
            - 策略改进定理保证 V^π_{k+1} ≥ V^π_k
            - 有限MDP中策略数量有限，单调递增序列必收敛
            - 收敛到全局最优策略（无局部最优问题）
    ...
    def value_iteration(self, max_iterations: int = 10000) -> AlgorithmResult:
        ...
        算法背景:
            值迭代由Bellman在1950年代提出，是求解MDP的另一种经典方法。
            该算法直接迭代贝尔曼最优方程，不显式维护策略，在收敛后从最优
            价值函数提取最优策略。值迭代可视为策略迭代的变体，其中策略评估
            被截断为单步更新。

        为什么提出值迭代:
            策略迭代的策略评估步骤需要完全收敛，计算代价高。值迭代跳过显式
            的策略评估，直接更新价值函数，每次迭代代价更低。这种"截断评估"
            的思想在大状态空间问题中特别有效。

        更新规则：
            V_{k+1}(s) = max_a Σ_{s'} P(s'|s,a) [R + γV_k(s')]

            这是贝尔曼最优方程的迭代形式，每次更新选择使当前价值估计最大的
            动作，隐式地进行了策略改进。
        ...
        收敛性分析:
            - 贝尔曼最优算子是γ-压缩映射
            - 根据Banach不动点定理必收敛到唯一不动点V*
            - 收敛速度: O(log(1/ε) / (1-γ))

        优点:
            1. 每次迭代简单高效，时间复杂度O(|S||A|)
            2. 不需维护策略，内存开销小
            3. 易于实现和并行化
            4. 收敛速度对γ不太敏感
